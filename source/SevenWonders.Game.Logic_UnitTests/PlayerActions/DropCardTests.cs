...
namespace GameLogic_UnitTests.PlayerActions
{
    public class DropCardTests
    {
        ...
        public void Setup()
        {
            ...
            m_droppedCardList = Substitute.For<ICardList>();
            m_droppedCardList.Cards.Returns(new List<Card>());
            ...
            m_gameContext.DroppedCardList.Returns(m_droppedCardList);
            m_ageHandler.CurrentAge.Returns(m_age);
            m_age.Composition.Returns(m_cardComposition);
            m_chooseGoodAction = new DropCard(m_current, m_opponent, m_card);
        }

        [Test]
        public void When_Constructor_Called_With_Null()
        {
            Assert.Throws<ArgumentNullException>(() => new DropCard(null, m_opponent, m_card));
            Assert.Throws<ArgumentNullException>(() => new DropCard(m_current, null, m_card));
            Assert.Throws<ArgumentNullException>(() => new DropCard(m_current, m_opponent, null));
        }
        ...
        private ICardList m_droppedCardList;
    }
}
