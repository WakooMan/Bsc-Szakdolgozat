...
namespace GameLogic_UnitTests.GameStructures.Factories
{
    public class CardCompositionFactoryTests
    {
        ...
        public void Setup()
        {
            ...
            m_cardCompositionFactory = new CardCompositionFactory(m_cardCompositionFileHandlerFactory, m_cardNodeFactory, m_eventManager);
        }

        [Test]
        public void When_Constructor_Called_With_Null()
        {
            Assert.Throws<ArgumentNullException>(() => new CardCompositionFactory(null, m_cardNodeFactory, m_eventManager));
            Assert.Throws<ArgumentNullException>(() => new CardCompositionFactory(m_cardCompositionFileHandlerFactory, null, m_eventManager));
            Assert.Throws<ArgumentNullException>(() => new CardCompositionFactory(m_cardCompositionFileHandlerFactory, m_cardNodeFactory, null));
        }
