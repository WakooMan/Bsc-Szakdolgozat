...
namespace GameLogic_UnitTests.Ages
{
    public class ThirdAgeTests
    {
        ...
        public void Setup()
        {
            ...
            m_thirdAge = new ThirdAge(m_eventManager, m_cardCompositionFactory, m_cardList, m_randomGenerator);
        }

        [Test]
        public void When_Constructor_Called_With_Null()
        {
            Assert.Throws<ArgumentNullException>(() => new ThirdAge(null, m_cardCompositionFactory, m_cardList, m_randomGenerator));
            Assert.Throws<ArgumentNullException>(() => new ThirdAge(m_eventManager, null, m_cardList, m_randomGenerator));
            Assert.Throws<ArgumentNullException>(() => new ThirdAge(m_eventManager, m_cardCompositionFactory, null, m_randomGenerator));
            Assert.Throws<ArgumentNullException>(() => new ThirdAge(m_eventManager, m_cardCompositionFactory, m_cardList, null));
        }
