...
namespace SevenWonders.Game.Logic.Handlers
{
    public class ChooseWonderHandler: IChooseWonderHandler
    {
        ...
        {
            ArgumentChecker.CheckPredicateForOperation(() => m_players.Count == 0 || m_wonders.Count == 0 || WondersChosen, "Wonder cannot be chosen if initialize is not called or all the wonders are chosen!");
