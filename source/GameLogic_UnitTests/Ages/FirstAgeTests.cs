...
namespace GameLogic_UnitTests.Ages
{
    public class FirstAgeTests
    {
        ...
        public void Setup()
        {
            ...
            m_firstAge = new FirstAge(m_eventManager, m_cardCompositionFactory, m_cardList);
        }

        [Test]
        public void When_Constructor_Called_With_Null()
        {
            Assert.Throws<ArgumentNullException>(() => new FirstAge(m_eventManager, null, m_cardList));
            Assert.Throws<ArgumentNullException>(() => new FirstAge(m_eventManager, m_cardCompositionFactory, null));
        }
