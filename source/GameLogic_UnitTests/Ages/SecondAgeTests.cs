...
namespace GameLogic_UnitTests.Ages
{
    public class SecondAgeTests
    {
        ...
        public void Setup()
        {
            ...
            m_secondAge = new SecondAge(m_eventManager, m_cardCompositionFactory, m_cardList);
        }

        [Test]
        public void When_Constructor_Called_With_Null()
        {
            Assert.Throws<ArgumentNullException>(() => new SecondAge(m_eventManager, null, m_cardList));
            Assert.Throws<ArgumentNullException>(() => new SecondAge(m_eventManager, m_cardCompositionFactory, null));
        }
